...
func FetchNewRuns(src runsSource, cache *Cache, userID int) (newRuns []Run, newMax int64, err error) {
	...
	// Посылка без вердикта (ejudge_status «нет значения»), отправленная
	// недавно, скорее всего ещё тестируется: не двигаем водяной знак выше
	// неё, иначе её поздний OK никогда не будет выкачан. Ограничение по
	// возрасту защищает от вечного перекачивания из-за посылок, у которых
	// вердикта не будет никогда; дубли исключает run_id в БД.
	for _, r := range newRuns {
		if r.EjudgeStatus == -1 && time.Since(r.CreateTime) < pendingHoldback && r.ID-1 < newMax {
			newMax = r.ID - 1
		}
	}
